"""
Environment server implementation for the Meta Data Curation Lab.
"""
...
class DataCurationGrader:
    ...
    def grade(self, state: DataCleanState) -> float:
        """Grades the current state by retrieving the score."""
        try:
...
class DataCurationEnv(Environment):
    ...
    def reset(
        ...
        return self._get_obs(
            f"Dataset '{task_id}' loaded. Goal: Improve model F1-score/R2."
        )

    def step(
        self, action: DataCleanAction, timeout_s=None, **kwargs
    ) -> DataCleanObservation:
        """Applies a data cleaning action to the environment."""
        ...
        try:
            if op == "finish":
                ...
                threshold = (
                    action.params.get("threshold", 50.0) if action.params else 50.0
                )
                cols_before = set(self._df.columns)
                self._df = preprocessing.drop_useless_columns(
                    self._df, threshold=float(threshold)
                )
                ...
                feedback = (
                    "[SUCCESS] Stage 3 complete. MUST move to Stage 4: impute_missing."
                )
    ...
    def _get_obs(
        self, message: str, reward: float = 0.0, done: bool = False
    ) -> DataCleanObservation:
    ...
    def _calculate_quality_score(self) -> float:
        """Trains a downstream model and evaluates its quality."""
        try:
            if self._df.empty or "target" not in self._df.columns:
                return 0.01

            x_feat = self._df.drop(columns=["target"])
            y = self._df["target"]

            mask = ~y.isna()
            x_final = x_feat[mask].copy()
            y_final = y[mask].copy()
            ...
            if is_regression:
                ...
                scores = cross_val_score(
                    clf, x_final, y_final, cv=3, scoring="f1_macro"
                )
                mean_score = float(np.nanmean(scores))
                if np.isnan(mean_score) or np.isinf(mean_score):
                    final_score = 0.01
    ...
    def _generate_mock_data(self, task_id: str) -> pd.DataFrame:
        ...
        if "breast_cancer" in task_id or "task_1" in task_id:
            ...
            df = pd.DataFrame(
                np.random.randn(n_rows, 4), columns=["a", "b", "c", "target"]
            )
